...
class Bot(BaseBot):
    """
    RL bot wrapper around pre-trained policies.
    """

    def __init__(self, board_size=15, algo="ppo", weights_path=None, device=None, deterministic=True):
        super().__init__()

        self.board_size = int(board_size)
        self.algo = str(algo).lower()
        if self.algo != "ppo":
            raise ValueError("Only 'ppo' is supported in this workspace.")
